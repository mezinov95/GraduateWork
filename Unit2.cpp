	...
	: TForm(Owner)   // Запускается при открытии приложения
{
	...
	Graph15->LabelY = "Коэффициент сшивки";
	Graph15->LabelX = "Амплитуда сигнала 12-го динода";
...
void LinkingCoefficient(short bek, short qsm, short pmt)
{
	if((EventBek[bek].maskahit[qsm]&(1<<pmt))!=0)
	{
		...
		if (dyn12 < 4096)   // Чтобы не вылезти за границы массива
		{
				...
				case MONAMPL_W:
					...
					if (type == PMT)
					{
						...
						if (CurrentDT.tm.minute != PreviousDT.tm.minute)
						{
							...
							double amp12 = MonAmpL[bek].sred[qsm][pmt][0];  // Амплитуда 12-го динода
							double amp9  = 10*MonAmpL[bek].sred[qsm][pmt][1];  // Амплитуда 9-го динода
							...
							double sigma_pedestal9 = MonPdsL[bek].sred[qsm][pmt][0];  // Сигма пьедестала 9-го динода
							double sigma_pedestal12 = MonPdsL[bek].sred[qsm][pmt][1];  // Сигма пьедестала 12-го динода
							DATA5[0][(int)linking_coefficient]++;
